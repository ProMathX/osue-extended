...
static int
cmpstringp_rev(const void *p1, const void *p2)
{
	return cmpstringp(p2, p1);
}

int
readLines(FILE *input, lines_t *lines)
{
	if (input == NULL) {
	...
	while ((read_size = getline(&line, &line_buffer_len, input)) != -1) {
	    ...
	    // initialize base capacity
			lines->capacity = lines->capacity ? lines->capacity *
			    2 : 128;
			lines->data = realloc(lines->data,
			    lines->capacity * sizeof(char *));
			if (lines->data == NULL)
...
void
sortAndPrint(lines_t *lines, FILE *output, bool reverse)
{
	qsort(lines->data, lines->count, sizeof(char *),
	    reverse ? cmpstringp_rev : cmpstringp);
